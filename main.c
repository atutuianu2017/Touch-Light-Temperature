...
int main(void)
{
    ...
    P1OUT &=~BIT4; P1OUT |= BIT5; __delay_cycles(200);
...
for (;;)
{
        ...
        //Temperature Controlling a LED on port 2.0
        //Code with deadzone to prevent flickering
        ...
        {
         //if temp is higher than 1.04 of temproom, turn LED2 on
              if(temp >= temproom * 1.04){
                  P2OUT |= BIT0;
                  __delay_cycles(200);}
         //if temp is lower  than 1.02 of temproom, turn LED2 off
        ...
        else
        {
            ...
            //Modify the code so that with each touch the direction changes and stays
            if(touch >= touchroom * 0.9) { touchflag = 0;}  // set motor FW direction
            if(touch <= touchroom * 0.7 && touchflag == 0) {P1OUT ^= BIT4|BIT5;  touchflag = 1; __delay_cycles(200);}  // set motor RV direction
        }
